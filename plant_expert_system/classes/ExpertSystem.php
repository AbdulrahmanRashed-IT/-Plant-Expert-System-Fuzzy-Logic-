...
class ExpertSystem {
    ...
    public function getAllDiseases() {
        try {
            // penyakit is a VIEW of hama with proper column names
            $query = "SELECT kode, nama_penyakit, nama_latin, deskripsi, tingkat_bahaya 
                    FROM penyakit 
                    ORDER BY kode";
    ...
    private function calculateFuzzyMembership($userSymptoms, $pestCode) {
        try {
            ...
            $query = "SELECT fr.nilai_keanggotaan, fr.bobot_kepentingan, g.bobot 
                FROM fuzzy_rules fr JOIN gejala g ON fr.kd_gejala = g.kd_gejala 
    ...
    public function getControlMethods($pestCode) {
        try {
            $query = "SELECT nama_obat, solusi FROM solusi WHERE kode_hama = ? ORDER BY nama_obat";
            $stmt = $this->conn->prepare($query);
    ...
    public function getAllPests() {
        try {
            $query = "SELECT kode, nama_hama, nama_latin, deskripsi, tingkat_bahaya 
                    FROM hama 
                    ORDER BY kode";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
